...
updateBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    const updatedEvents = await updateEventsWithChat(syllabusInput.value, corrections, parsedEvents);
...
addToCalendarBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    showStatus(`Successfully added ${parsedEvents.length} events to Google Calendar!`, 'success');
...
async function parseSyllabusWithGroq(syllabusText) {
  ...
  if (!apiKey) {
    throw new Error('Groq API key not set. Please add it in extension options.');
  }
...
async function updateEventsWithChat(originalSyllabus, corrections, currentEvents) {
  const result = await chrome.storage.sync.get(['groqApiKey']);
  const apiKey = result.groqApiKey;
...
async function addEventToGoogleCalendar(token, event) {
  const calendarEvent = {
    summary: event.title,
    description: `${event.className} - ${event.eventType}${event.description ? '\n\n' + event.description : ''}`,
    start: {
